...
const getVehicles = async function (req, res, next) {
    var vehicles = [];
    const responseVehicles = await pool.query('SELECT v.*, NULL AS v_owner_id, u.user_name,u.user_qr_id, u.phone_no, u.acc_status_active, vt.v_type_name FROM (vehicles AS v LEFT OUTER JOIN users AS u ON v.v_owner_id=u.user_id FULL OUTER JOIN vehicles_type AS vt ON v.v_type_id = vt.v_type_id) WHERE u.user_type=$1', ["owner"]);
    try {
        if (res.status(200)) {
...
const getVehicleByID = async function (req, res, next) {
    ...
    var vehicles = [];
    const responseVehicles = await pool.query('SELECT v.*, NULL AS v_owner_id, u.user_name,u.user_qr_id, u.phone_no, u.acc_status_active, vt.v_type_name FROM (vehicles AS v LEFT OUTER JOIN users AS u ON v.v_owner_id=u.user_id FULL OUTER JOIN vehicles_type AS vt ON v.v_type_id = vt.v_type_id) WHERE u.user_type=$1 AND v.v_id=$2', ["owner", vID]);
    try {
        if (res.status(200)) {
            if (responseVehicles.rowCount != 0 && responseVehicles.rowCount != null) {
                let promise = new Promise(async (resolve, reject) => {
